...
INTERVENTION_GRAPH = _load_module('charon_intervention_graph_loop', _THIS_DIR / 'intervention_graph.py')
CONVERSATION_INDEX = _load_module('charon_conversation_index_loop', _THIS_DIR / 'conversation_index.py')
AGENT_RUNTIME = _load_module('charon_agent_runtime_loop', _THIS_DIR / 'agent_runtime.py')
AGENT_LIFECYCLE = _load_module('charon_agent_lifecycle_loop', _THIS_DIR / 'agent_lifecycle.py')
LLM_ADAPTER = _load_module('charon_llm_adapter_loop', _THIS_DIR / 'llm_adapter.py')
BOUNDARY_RUNTIME = _load_module('charon_boundary_runtime_loop', _THIS_DIR / 'boundary_runtime.py')
SHADE_ORCH = _load_module('charon_shade_orchestrator_loop', _THIS_DIR / 'shade_orchestrator.py')
AGENT_POLICY = _load_module('charon_agent_policy_loop', _THIS_DIR / 'agent_policy.py')
GOAL_RUNTIME = _load_module('charon_goal_runtime_loop', _THIS_DIR / 'goal_runtime.py')
...
def run_loop(state_dir: Path, stop_file: Path, max_consecutive_failures: int, sleep_sec: float, max_cycles: int = 0, debug_trace: bool = False):
    ...
    while True:
        ...
        if cycles - last_heartbeat_cycle >= heartbeat_interval:
            ...
            try:
                from consolidation import load_config as _load_consol_config, should_run as _consol_should_run, run_consolidation as _run_consolidation
        ...
        if not pending:
            ...
            auto_task = None
            try:
                from autonomous import load_autonomous_config, self_assign_next_task
