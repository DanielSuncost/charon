...
def propose_goal(
    ...
    *,
    ...
    # Sync to SQLite
    try:
        from goal_runtime import _use_store, _get_db, _db_project_upsert, _db_session_upsert
        if _use_store():
...
def get_goals_by_status(state_dir: Path, *, project: str, status: str) -> list[dict]:
    ...
    from goal_runtime import _safe_id, _project_path, _read_json, _default_project_doc
    project_id = _safe_id(project or 'default-project', 'project')
...
def _update_goal_status(state_dir: Path, *, project: str, goal_id: str,
    ...
    if found:
        ...
        _write_json(ppath, proj)
        try:
            from goal_runtime import _use_store, _get_db, _db_project_upsert
            if _use_store():
