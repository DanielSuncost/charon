...
def _build_task_system_prompt(state_dir: Path, agent: dict, task: dict) -> str:
    ...
    if contract_id and agent.get('role') == 'shade':
        try:
            ...
            _orch_path = Path(__file__).resolve().parent / 'shade_orchestrator.py'
            _spec = _ilu.spec_from_file_location('_shade_orch_prompt', _orch_path)
            _mod = _ilu.module_from_spec(_spec)
            _spec.loader.exec_module(_mod)
            contract = _mod.get_contract(state_dir, contract_id)
        except Exception:
            pass
