...
Run `bash skills/manim-video/scripts/setup.sh` once to install/verify all dependencies into Charon's environment. It installs the system libs (cairo, pango, pkg-config, ffmpeg) and `manim` into Charon's `.venv`, on macOS (Homebrew) and Ubuntu (apt). Re-run with `--check` to verify without installing.

LaTeX is optional — pass `--with-latex` only if you need animated math equations (`MathTex`); most product/launch videos don't. Requires Python 3.10+, Manim CE v0.20+, and ffmpeg. Reference docs tested against Manim CE v0.20.1.
