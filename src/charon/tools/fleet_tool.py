...
def execute_fleet_send(params: dict, context: Any) -> Any:
    from charon.tools import ToolResult
    try:
        from charon.fleet.fleet_sync import send_to_remote_agent
    except ImportError:
...
def execute_fleet_history(params: dict, context: Any) -> Any:
    from charon.tools import ToolResult
    try:
        from charon.fleet.fleet_sync import get_remote_agent_history
    except ImportError:
        return ToolResult(content='Fleet sync not available.', is_error=True)
...
def execute_fleet_onboard(params: dict, context: Any) -> Any:
    from charon.tools import ToolResult
    try:
        from charon.fleet.remote_onboard import full_onboard, auto_configure_fleet
    except ImportError:
