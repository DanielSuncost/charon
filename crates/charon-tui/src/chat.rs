...
impl ChatState {
    ...
    fn command_suggestions(&self, input: &str) -> Vec<MenuItem> {
        ...
        let mut items = vec![
            ...
            item("/skills", "List bundled + user skills (e.g. manim-video)"),
            item("/skills view manim-video", "Read a skill before using it"),
            item("/skills setup manim-video", "Install a skill's dependencies"),
            item("/skills setup manim-video --check", "Verify a skill's dependencies without installing"),
            item("/skills setup manim-video --with-latex", "Install skill deps including LaTeX (math equations)"),
            item("/reel", "Generate the Charon feature video reel (parallel swarm)"),
            item("/reel plan <path> --reel <name>", "Direct a reel for any app: auto-write its feature manifest"),
            item("/reel build --reel <name>", "Render a named app reel after reviewing its manifest"),
            item("/reel list", "List the feature videos in the reel manifest"),
            item("/reel build", "Spawn shades to render all feature videos"),
            item("/reel build video memory", "Render only specific features by slug"),
            item("/reel stitch", "Stitch rendered clips into results/videos/charon-reel.mp4"),
            item("/reel judge video", "Optimize a clip with the aesthetic judge loop"),
