...
### Changed
- **`gpt-6-astra` is selectable with its real effort ladder.** The Codex picker offers it first; `/setup effort` / `/effort` (new in the TUI backend, extended in the CLI) accept `off minimal low medium high xhigh max ultra`, show the ladder for the current model, and say when a model tops out lower. `max` is no longer a silent alias for `high`. The Codex transport sends the requested level clamped to what *that* model accepts (`charon.providers.effort`; verified live — the endpoint rejects anything outside a model's set with a 400): on astra `xhigh` and `max` go through instead of being folded into `high`, `ultra` (a Codex-CLI delegation mode, not a wire value) is sent as `max`, `minimal` folds to `low` as before; on `gpt-5.5` `max` becomes `xhigh`; unknown models keep the old `low…high` ladder. Anthropic thinking budgets for `max`/`ultra` no longer fall back to the medium budget.
- **Workers pick their own reasoning effort.** `route_shade_effort` in the model registry chooses a shade's effort from `task_complexity` (`simple→low`, `normal→medium`, `complex→high`, configurable via `effort_by_complexity` in `model_registry.json`) and steps it down one notch — never below `low` — on the same ≥75 % budget signal that already moves the model tier, so a probe never burns high and a hard contract under a starved tree still gets medium. `shade_effort_mode: same` (or `CHARON_SHADE_EFFORT_MODE=same`) makes workers inherit the session's level. The user's own session keeps whatever they chose.
